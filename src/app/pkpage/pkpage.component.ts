...
export class PkpageComponent {
  ...
  loadPokemons() {
    ...
    this.databaseService.getPokemonList(this.itemsPerPage, offset).subscribe({
      ...
      error: (err) => {
        this.error =
          'Error al cargar los pokemones. Por favor, intenta de nuevo.';
        this.loading = false;
  ...
  selectPokemon(id: number) {
    ...
    this.selectionAudio
      .play()
      .catch((e) => console.log('Error al reproducir audio de selección:', e));
  }
  ...
  applyFilter(filter: string) {
    ...
    if (index !== -1) {
      ...
      if (
        filter === 'ascendente' &&
        this.activeFilters.includes('descendente')
      ) {
        this.activeFilters.splice(this.activeFilters.indexOf('descendente'), 1);
      } else if (
        filter === 'descendente' &&
        this.activeFilters.includes('ascendente')
      ) {
  ...
  loadAllPokemonForFiltering() {
    ...
    this.databaseService.getPokemonList(limit, 0).subscribe({
      ...
      error: (err) => {
        this.error =
          'Error al cargar todos los pokemones. Por favor, intenta de nuevo.';
        this.loading = false;
  ...
  loadPokemonDetails() {
    ...
    const pokemonToProcess =
      this.allPokemonList.length > 0 ? this.allPokemonList : this.pokemonList;
    ...
    const detailsRequests = pokemonToProcess.map((pokemon) => {
      ...
      return this.databaseService.getPokemonDetails(pokemon.id).pipe(
        catchError((error) => {
          console.error(
            `Error al cargar detalles del Pokémon ${pokemon.id}:`,
            error
          );
          return of(null);
        })
    ...
    this.activeFilters.forEach((filter) => {
      switch (filter) {
        ...
        case 'ataque':
          filtered.sort((a, b) => {
            const aAttack =
              a.stats?.find((s: any) => s.name === 'attack')?.value || 0;
            const bAttack =
              b.stats?.find((s: any) => s.name === 'attack')?.value || 0;
        ...
        case 'defensa':
          filtered.sort((a, b) => {
            const aDefense =
              a.stats?.find((s: any) => s.name === 'defense')?.value || 0;
            const bDefense =
              b.stats?.find((s: any) => s.name === 'defense')?.value || 0;
        ...
        case 'velocidad':
          filtered.sort((a, b) => {
            const aSpeed =
              a.stats?.find((s: any) => s.name === 'speed')?.value || 0;
            const bSpeed =
              b.stats?.find((s: any) => s.name === 'speed')?.value || 0;
            ...
            filtered = filtered.filter((pokemon) => {
              ...
              if (pokemonDetail && pokemonDetail.types) {
                return pokemonDetail.types.some(
                  (t: any) => t.type.name === this.selectedType
                );
              }
              return false;
            });
